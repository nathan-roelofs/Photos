...
<body>
    ...
    <main>
        ...
        <div id="addPhotoModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);align-items:center;justify-content:center;z-index:1300;padding:20px">
            <div style="max-width:600px;width:100%;background:var(--card-bg);border-radius:12px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.6);border:1px solid var(--border)">
                ...
                <div style="padding:24px;overflow-y:auto;max-height:calc(90vh - 100px)">
                    <form id="addPhotoForm" method="POST" action="{{ route('photos.store') }}" enctype="multipart/form-data">
                        @csrf
                        <div style="margin-bottom:20px">
                            ...
                            <select name="album_id" style="width:100%;padding:10px 12px;border:1px solid var(--border);border-radius:8px;background:#ffffff;color:var(--text-primary);box-sizing:border-box">
                                @foreach($albums as $alb)
                                    <option value="{{ $alb->id }}">{{ $alb->titre }}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <section id="albumPanels" style="max-width:1100px;margin:0 auto;padding:1rem;">
            @foreach($albums as $index => $album)
                <div class="album-panel" data-index="{{ $index }}" data-album-id="{{ $album->id }}" data-creation="{{ $album->creation }}" style="display:none;margin-bottom:2rem;padding:0;overflow:hidden;">
                    <header style="display:flex;align-items:center;justify-content:space-between;padding:24px;border-bottom:1px solid var(--border);border-radius:12px 12px 0 0;">
                        <div style="display:flex;flex-direction:column;gap:8px">
                            <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap">
                                <h2 style="margin:0">{{ $album->titre }}</h2>
                    ...
                    @else
                        <div class="gallery--columns" style="padding:1rem">
                            @foreach($album->photos as $photo)
                                <figure class="card" data-id="{{ $photo->id }}" data-tags="{{ $photo->tags->pluck('id')->join(',') }}" data-title="{{ htmlspecialchars($photo->titre, ENT_QUOTES) }}" data-note="{{ $photo->note ?? 0 }}">
